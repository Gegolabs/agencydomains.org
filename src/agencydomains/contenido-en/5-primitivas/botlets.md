...
### The three Botlet tests — minion, specialty, and life

The criteria above say when it is *worth* generating a Botlet. What is missing is the prior question, which is one of identity, not convenience: faced with an artifact that already exists — a program, a configuration, a report, an instrument — is it a Botlet or something else? The question is not academic. Two readers with this very chapter in front of them can disagree within minutes on whether each Information Product of Mira is a Botlet or whether Mira is, and the disagreement is not settled by rereading the definition: it is settled with tests. There are three, and an artifact is a Botlet only if it passes all three.

**The minion test.** A Botlet is a minion with a trade. The question is what it does with its life. If it **executes** a task over and over — each execution being one case of its life — it is a minion. If it is only **applied**, read, or consumed, it is not the minion: it is what a minion handles. The short form of the question is *does this execute, or is it applied?* An Information Product executes: every pulse of a discrete Botlet and every subscription to a continuous one is an execution, and its life is the succession of those executions adapting to a source that changes. An assessment instrument is applied: someone sits it, and the instrument did nothing. The subtlety this test protects is that the cases of a minion's life are its executions over time, not different assignments: a worker at a single station on the line has a full trade even though they always do the same thing. And the corollary: when one counts as many Botlets as there are cases, look at whether those cases execute or are applied; if they are applied, one is counting instruments.

**The specialty test — `RISC` versus `CISC`.** It decides granularity when there are two ways to count. A `CISC` architecture offers few complex instructions, each doing a lot; a `RISC` one, many simple instructions with a single function each, which compose. The canon chooses `RISC` — the same preference that made Unix a collection of small tools chained together —: **one Botlet, one specialty.** When the doubt is between "one Botlet that handles N things" and "N Botlets that each do one thing", the canonical answer is the latter, and what keeps N Botlets from being N programs is the shared engine: the platform proto-Botlet. A Mira conceived as a single Botlet administering N Information Products would be `CISC`: its identity, its maturity, its fallback, and its quality contract would be averages of a monolith, and when one source changed the whole would degrade. One Botlet per Information Product is `RISC`: each one falls, matures, and retires on its own. The test does not say "always smaller"; it says **one specialty per unit**.
