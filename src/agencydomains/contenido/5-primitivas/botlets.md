...
### Las tres pruebas del Botlet — minion, especialidad y vida

Los criterios anteriores dicen cuándo *conviene* generar un Botlet. Falta la pregunta previa, que es de identidad y no de conveniencia: ante un artefacto que ya existe —un programa, una configuración, un informe, un instrumento—, ¿es un Botlet o es otra cosa? La pregunta no es académica. Dos lectores con este mismo capítulo delante pueden discrepar en minutos sobre si cada Producto de Información de Mira es un Botlet o si lo es Mira, y la discrepancia no se resuelve releyendo la definición: se resuelve con pruebas. Son tres, y un artefacto es Botlet solo si pasa las tres.

**La prueba del minion.** Un Botlet es un minion con oficio. La pregunta es qué hace en su vida. Si **ejecuta** una tarea una y otra vez —cada ejecución es un caso de su vida— es un minion. Si solo **se aplica**, se lee o se consume, no es el minion: es lo que un minion maneja. La forma corta de la pregunta es *¿esto se ejecuta o se aplica?* Un Producto de Información se ejecuta: cada pulso de un Botlet discreto y cada suscripción a uno continuo es una ejecución, y su vida es la sucesión de esas ejecuciones adaptándose a una fuente que cambia. Un instrumento de evaluación se aplica: alguien lo rinde, y el instrumento no hizo nada. La sutileza que esta prueba protege es que los casos de la vida de un minion son sus ejecuciones en el tiempo, no encargos distintos: un obrero en una sola estación de la línea tiene un oficio completo aunque haga siempre lo mismo. Y el corolario: cuando se cuentan tantos Botlets como casos, hay que mirar si esos casos se ejecutan o se aplican; si se aplican, se están contando instrumentos.

**La prueba de especialidad — `RISC` contra `CISC`.** Decide la granularidad cuando hay dos formas de contar. Una arquitectura `CISC` ofrece pocas instrucciones complejas que hacen mucho cada una; una `RISC`, muchas instrucciones simples con una sola función, que componen. El canon elige `RISC` — la misma preferencia que hizo de Unix una colección de herramientas pequeñas que se encadenan —: **un Botlet, una especialidad.** Cuando la duda es entre «un Botlet que maneja N cosas» y «N Botlets que hacen una cosa cada uno», la respuesta canónica es la segunda, y lo que impide que N Botlets sean N programas es el motor compartido: el proto-Botlet platafórmico. Un Mira concebido como un solo Botlet que administra N Productos de Información sería `CISC`: su identidad, su madurez, su fallback y su contrato de calidad serían promedios de un monolito, y cuando una fuente cambiara se degradaría el conjunto. Un Botlet por Producto de Información es `RISC`: cada uno cae, madura y se retira solo. La prueba no dice «más pequeño siempre»; dice **una especialidad por unidad**.
