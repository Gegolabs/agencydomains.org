...
Con los **Agentlets** (Capítulo 5 §7), el pilar de Validación gana además una sede en la **Capa 3**: la inferencia acotada de cada Agentlet cursa por el punto de control `cognition_call` del handle del Botler, y ahí aplican los mecanismos de este pilar — detección de alucinaciones sobre sus veredictos, validación de salidas estructuradas contra el contrato del charter, DLP y tokenización sobre lo que entra y sale del modelo. La corrección estadística que los Agentlets introducen en la capa de la memoria muscular queda así confinada a unidades declaradas y auditables — la disciplina de delgadez que la Bounded Concerns Architecture enseña, ejercida dentro del Mundo Agentivo.
