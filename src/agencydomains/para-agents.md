...
- **Botlet tests (minion · specialty · life)** — the three identity tests for declaring something a Botlet: does it execute, or is it applied? · one specialty per unit (`RISC`) · does it have a life cycle, or only a version? A `PI` passes all three; an assessment instrument, none. Ch 5 §2.
