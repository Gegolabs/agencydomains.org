...
- **El Instrumento de Diagnóstico** (Anexo A del libro, incorporado en v1.1) condensa las diecisiete preguntas diagnósticas de IRIS y MOTOR en forma aplicable, con la regla para situarse (la posición es la última pregunta respondida «sí» con evidencia) y la lectura del par de coordenadas. Se publica para ser usado, incluido el trabajo de consultoría remunerado, con atribución.
...
- **Estado v1.1**: el libro está publicado como primera edición en agencydomains.org. Desde esta versión los términos, las estructuras y la numeración son estables: lo que cambie se anunciará en el `CHANGELOG.md` del libro.
