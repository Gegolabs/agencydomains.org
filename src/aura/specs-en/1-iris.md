...
<!-- standalone -->
For 30 years, the information-management paradigm was **"people go to the data"**: you build a warehouse, set up dashboards, train users, and hope that someone looks at the right report at the right moment and makes the right decision. The entire model rests on human attention as the bottleneck.

Artificial intelligence is changing that equation, but not all AI changes it the same way. The industry distinguishes between two horizons. In the *agentic* world, agents complement existing applications: employees keep opening Excel, Salesforce, Power BI — now with copilots that help them work faster. It is incremental evolution. In the **agentive** world, agents replace the traditional interfaces: employees stop opening applications and interact directly with agents that execute tasks on their behalf. The applications may live on as infrastructure, but the interface has collapsed. It is fundamental transformation. The frontier between the two worlds — do your employees still open applications to do their work? — is what we call **the Nadella Line**.

**Why "agentive" and not "agentic"?** Because they are not the same thing. The industry has adopted "agentic AI" to describe agents that assist — copilots, assistants, smarter tools. But this model measures the trajectory toward something qualitatively different: a world where agents do not complement the way of working but transform it. Using "agentic" would position this model on the horizon of incremental evolution. Using "agentive" positions it on the horizon of fundamental transformation — which is what the model's advanced stages describe.

Agentive AI inverts the flow of information: **"intelligence goes to the people — and acts on their behalf."** A system of agents monitors, interprets, decides, and executes within the limits the organization has defined, and escalates to the human only when appropriate. This shift is not an incremental improvement to the analytics stack — it is a change of operating model.

The impact manifests along three simultaneous axes:

**From consuming information to governing agents.** People cease to be consumers of dashboards and become designers of rules and supervisors of autonomous systems. The CFO does not review a cashflow dashboard — they define the thresholds and protocols that a financial agent executes autonomously.

**From architecture for humans to architecture for agents.** Data warehouses optimized for SQL give way to semantic layers where agents reason, knowledge graphs for autonomous inference, and real-time data flows. Data quality is no longer measured by "cleanliness" but by "actionability."

**From access governance to autonomy governance.** The question is no longer "who can see which data" but "what can an agent do, under what conditions, and with what traceability." Organizations that fail to resolve this will not be able to scale agents beyond isolated pilots.

The trajectory of this transformation is articulated through the **Value Pyramid**: from organizations focused on the DATA layer (accumulating and structuring), through INFORMATION (contextualizing), then KNOWLEDGE (analyzing and understanding in real time), up to ACTION (executing autonomously within governed frameworks). The most disruptive moment occurs when the cost of an analytical question collapses from weeks to seconds — what we call **the Quantum Leap** — enabling the organization to move from rationing its questions to having elastic, unlimited analytical capacity.
<!-- /standalone -->
...
<!-- standalone -->
The distinction between *accessing* information in real time and *reacting* in real time is central to understanding the trajectory this model measures.

An **online enterprise** has its data current, its dashboards up to date, its information accessible — but it depends on a human looking, interpreting, and deciding. A **real-time enterprise** does not merely access: it detects, interprets, decides, and acts — continuously and autonomously.
<!-- /standalone -->
<!-- libro
...
![The IRIS map — 10 stages, 5 levels, the mapping to the Value Pyramid and the Quantum Leap](figuras/a04-iris-mapa.png)
...
<!-- standalone -->
The transformation described in *Visión de AURA — Arquitectura Unificada de Referencia Agentiva* (ultraBASE, February 2026) provides the conceptual framework that both models operationalize.
<!-- /standalone -->
<!-- libro
